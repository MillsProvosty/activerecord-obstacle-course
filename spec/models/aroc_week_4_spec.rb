...
# ----------------------
  ...
  xit '22. calculates the total sales for all but one user' do
    ...
    # end.select{|i| !i.nil?}
      # total_sales = orders.map(&:amount).nject(:+)
    # -----------------------------------------------------------
  ...
  xit '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where(user_id: @user_2.id).includes(:items).where("order_items.item_id = ?", @item_4.id)
    order = Order.joins(:order_items).where(user_id: @user_2.id).where("order_items.item_id = ?", @item_4.id)
    order = Order.joins(:order_items).where(user_id: @user_2.id, order_items: {item_id: @item_4.id})
    order = Order
      .joins(:order_items)
